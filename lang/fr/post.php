<?php

declare(strict_types=1);

return [
    'breadcrumb_home' => 'Accueil',

    'page_title' => 'Articles',
    'page_description' => 'Articles pratiques sur la nutrition, une alimentation saine et le bien-être — soutenus par les dernières recherches en santé.',
    'seo_title' => 'Articles | GLC AI Platform',
    'seo_description' => 'Articles sur la nutrition, l\'alimentation saine, la planification des repas et des conseils bien-être pour faire des choix alimentaires plus judicieux.',
    'meta_keywords' => 'articles nutrition, alimentation saine, gestion du diabète, planification des repas, contrôle glycémique, bien-être, recherche en santé',
    'og_image_alt' => 'GLC AI Platform - Articles nutrition et santé',

    'read_article' => 'Lire l\'article',
    'min_read' => ':minutes min de lecture',
    'no_articles_title' => 'Pas encore d\'articles',
    'no_articles_description' => 'Revenez bientôt — nous ajoutons régulièrement du contenu pour vous aider à vivre plus sainement.',

    'read_this_article_in' => 'Lire cet article en',

    'cta_index_title' => 'Besoin de conseils nutritionnels personnalisés ?',
    'cta_index_description' => 'Notre nutritionniste IA peut analyser vos repas, prédire l\'impact sur la glycémie et créer un plan personnalisé — adapté à vos objectifs de santé.',
    'cta_index_button' => 'Demander à notre nutritionniste IA',

    'cta_show_title' => 'Des questions sur :topic ?',
    'cta_show_description' => 'Notre nutritionniste IA peut vous aider à comprendre comment ce sujet est lié à vos objectifs de santé personnels et à la gestion de votre glycémie.',
    'cta_show_button' => 'Demander à notre nutritionniste',

    'log_in' => 'Se connecter',
    'get_started' => 'Commencer',

    'category_product_updates' => 'Mises à jour produit',
    'category_nutrition_tips' => 'Conseils nutrition',
    'category_recipes' => 'Recettes',
    'category_research' => 'Recherche',
    'category_lifestyle' => 'Mode de vie',
    'category_diabetes_management' => 'Gestion du diabète',

    'category_title_product_updates' => 'Mises à jour produit et annonces',
    'category_title_nutrition_tips' => 'Conseils nutrition : Mangez mieux, vivez mieux',
    'category_title_recipes' => 'Recettes saines : Repas et collations équilibrés',
    'category_title_research' => 'Recherche en santé et science de la nutrition',
    'category_title_lifestyle' => 'Mode de vie et bien-être',
    'category_title_diabetes_management' => 'Gestion du diabète et contrôle de la glycémie',

    'category_desc_product_updates' => 'Nouvelles d\'GLC AI Platform — nouvelles fonctionnalités, mises à jour et notre mission de rendre la santé accessible à tous.',
    'category_desc_nutrition_tips' => 'Conseils nutritionnels fondés sur des preuves — comprendre les macros, l\'indice glycémique et faire des choix alimentaires plus judicieux.',
    'category_desc_recipes' => 'Des recettes délicieuses et saines, conçues pour vous nourrir sans sacrifier la saveur.',
    'category_desc_research' => 'Résumés des dernières recherches en science de la nutrition, santé métabolique et bien-être.',
    'category_desc_lifestyle' => 'Conseils pour l\'exercice, un meilleur sommeil et la gestion du stress — les fondements d\'une vie saine.',
    'category_desc_diabetes_management' => 'Guides pratiques pour gérer le diabète, surveiller la glycémie et prévenir les pics de glycémie.',
];
