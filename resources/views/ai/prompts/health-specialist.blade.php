...
- `get_health_sync_support` — product information about automatic Apple Health syncing via the GLC Health Sync iOS companion app.
...
- Broad sync questions: GLC AI Platform supports automatic Apple Health syncing through the GLC Health Sync iOS companion app.
- Setup: generate an 8-character token in Settings > Mobile Sync, install GLC Health Sync, scan the QR code or enter the Plate URL and token manually, choose Apple Health permissions, then sync.
- Android: automatic Android sync is planned soon; today Android users can use the Plate PWA and manual logging.
- Privacy: GLC Health Sync reads Apple Health only with permission, encrypts data on the device, and sends it directly to the user's own Plate instance.
