...
<x-default-layout>
    ...
    <div class="print:hidden">
        <div class="mx-auto max-w-4xl px-6 py-8">
            ...
            >
                ...
                <span>{{ __('common.grocery_list_print.back_to_list') }}</span>
            </a>
            ...
            <div class="mb-6">
                ...
                <p class="mt-2 text-slate-600 dark:text-slate-400">
                    {{ __('common.grocery_list_print.printable_description', ['days' => $mealPlan->duration_days, 'count' => $groceryList->items->count()]) }}
                </p>
            </div>
            ...
            <div class="mb-8 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950">
                <p class="mb-2 text-sm text-blue-900 dark:text-blue-100">
                    <strong>{{ __('common.grocery_list_print.desktop_only') }}</strong> {{ __('common.grocery_list_print.desktop_only_description') }}
                </p>
                <p class="text-sm text-blue-800 dark:text-blue-200">
                    <strong>{{ __('common.grocery_list_print.to_download') }}</strong> {{ __('common.grocery_list_print.to_download_description') }}
                </p>
            </div>
            ...
            >
                ...
                {{ __('common.grocery_list_print.print_download_pdf') }}
            </button>

            <div class="mt-8 rounded-lg bg-slate-100 p-4 dark:bg-slate-800">
                <p class="mb-4 text-center text-sm text-slate-600 dark:text-slate-400">{{ __('common.grocery_list_print.preview_scroll') }}</p>
            </div>
        </div>
    </div>
    ...
    >
        ...
        <header class="mb-6 border-b-2 border-black pb-4 print:border-black print:dark:border-black">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right text-sm text-slate-600 print:text-slate-600 print:dark:text-slate-600">
                    <p>{{ __('common.grocery_list_print.day_plan', ['days' => $mealPlan->duration_days]) }}</p>
                    <p>{{ __('common.grocery_list_print.items_count', ['count' => $groceryList->items->count()]) }}</p>
                    <p>{{ __('common.grocery_list_print.generated', ['date' => $groceryList->created_at->format('M j, Y')]) }}</p>
                </div>
            </div>
        </header>
        ...
        <footer class="mt-8 border-t border-slate-300 pt-4 text-center text-xs text-slate-500 print:mt-6 print:border-slate-400 print:dark:border-slate-400">
            <p>{{ __('common.grocery_list_print.generated_by', ['date' => now()->format('F j, Y')]) }}</p>
            <p class="mt-1">{{ __('common.grocery_list_print.tagline') }}</p>
        </footer>
    </article>
</x-default-layout>
